...
module.exports = function cities(req, res) {
    ...
    if(req.method === 'POST') {
        ...
        cities.findOne({ _id: cityId })
            ...
            .catch(console.log);
    } else if(req.method === 'GET' && req.url.query.state) {
        const queryState = req.url.query.state;

        cities.find({ state: queryState }).toArray()
            .then(cities => cities.length === 0 ? notFound(req, res) : res.end(JSON.stringify(cities)))
            .catch(console.log);
